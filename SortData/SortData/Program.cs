using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;

namespace SortData
{
    public class Comparer : IComparer<string>
    {
        public int Compare(string x, string y)
        {
            if (x.StartsWith("Matchday"))
            {
                return -1;
            }
            if (y.StartsWith("Matchday"))
            {
                return 1;
            }

            return Convert.ToInt32(x.Split(",")[0]) - Convert.ToInt32(y.Split(",")[0]);
        }
    }


    class Program
    {
        static void Main(string[] args)
        {
            string data = @"D:\Documents\Master\Hochschule Fulda\Module\Masterarbeit\Implementierungen\Data\";

            foreach (string csv in Directory.GetFiles(data, "*_*.csv"))
            {
                string season = Path.GetFileNameWithoutExtension(csv);

                string[] lines = File.ReadAllLines(csv);

                List<string> games = new();

                foreach (string line in lines)
                {
                    string l = line.Replace(",", ";");
                    games.Add(l);
                }

                IEnumerable<string> query = lines.OrderBy(a => a, new Comparer());

                string newFile = null;

                foreach (string line in query)
                {
                    newFile += line + "\n";
                }

                File.WriteAllText(data + season + "_new.csv", newFile);
            }
        }
    }
}
